...
export default function MemberRoutines() {
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      if (res.data.success) {
        setRoutines((prev) =>
          prev.filter((routine) => routine.routineId !== id)
        );
      }
    } catch (err: any) {
      console.error("Error deleting routine:", err);
      alert(err.response?.data?.error || "Failed to delete routine");
    }
  };
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 w-fit">
          {filteredRoutines.length === 0 && (
            <p className="text-gray-500 col-span-full">No routines found.</p>
          )}
          ...
          {filteredRoutines.map((routine) => (
            <RoutineCard
              ...
              creator={routine.createdBy?.userName || "Unknown"}
